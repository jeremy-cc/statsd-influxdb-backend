...
function InfluxdbBackend(startupTime, config, events) {
    ...
    if (self.version >= 1) {
        ...
        self.log("added v1.0")
    } else if (self.version < 1 && self.version >= 0.11) {
...
/**
 * Extract tags from key and append to the end
 *
 * foo.bar,hello=world,baz=qux:1|c
 *
 */
...
InfluxdbBackend.prototype.processFlush = function (timestamp, metrics) {
  ...
  var key, timerKey;
  var statsPrefixRegexp = new RegExp('^' + self.prefixStats + '\\.');

    /* Convert timestamp from seconds to milliseconds. */
    timestamp = (timestamp * 1000);
    for (key in counters) {
        /* Do not include statsd counters. */
        if (!self.includeStatsdMetrics && key.match(statsPrefixRegexp)) {
            continue;
        }

        var value = counters[key];
        var k = influxKey(key, 'counter');
    ...
    for (key in timerData) {
        ...
        if (timerMetrics.histogram) {
            ...
            for (histoKey in histoMetrics) {
                ...
                var k = influxKey(key, 'timer.histogram.' + histoKey);
        ...
        for (timerKey in timerMetrics) {
            ...
            var k = influxKey(key, 'timer' + '.' + timerKey);
...
InfluxdbBackend.prototype.httpPOST_v08 = function (points) {
    ...
    var self = this;
    var query = {u: self.user, p: self.pass, time_precision: 'ms'};
    var protocolName = self.protocol == http ? 'HTTP' : 'HTTPS';
    var startTime;
...
InfluxdbBackend.prototype.httpPOST_v09 = function (points) {
    ...
    var self = this;
    var query = {u: self.user, p: self.pass};
    var protocolName = self.protocol == http ? 'HTTP' : 'HTTPS';
    var startTime;
    ...
    req.on('response', function (res) {
        ...
        if (status >= 400) {
            ...
            res.on('data', function (chunk) {
                self.log(protocolName + ' response: ' + chunk);
            });
        }
    });
...
InfluxdbBackend.prototype.assembleEvent_v11 = function (name, events) {
    var payload = name + ' value=' + events[0]['value'] + ' ' + (events[0]['time'] * 1000000);
    return payload
};

/**
 * @ Vita Zhao add new version httpPOST of version 0.11 with http call
 */
...
InfluxdbBackend.prototype.httpPOST_v1 = function (points) {
    ...
    self.influxdbStats.payload = payload;
    ...
    self.logDebug(function () {
        return 'payload is' + self.influxdbStats.payload
    });
